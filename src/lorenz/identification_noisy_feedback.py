...
state_data_noisy.iloc[:,:] = add_noise(state_data.values, [0.02, 0.2, 0.2])
state_data_noisy.reset_index(drop=True, inplace=True)

filter = SpectralFilter(state_data_noisy, dt, plot=True)
filter.find_cutoff_frequencies(offset=0, std_thresh=2)
