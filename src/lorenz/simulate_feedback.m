...
savefile = 'lorenz_sim_feedback_val.csv';
...
% control_law = @(x) [0, 0, 0];
control_law = @(x) [-0.25*(x(1)-20), -0.25*(x(2)-20), -0.25*(x(3)-24)];
...
odefun = @(t, x)lorenz_2(t, x, u_fun, params);

x0 = [-10, -10, 15]'; % initial conditions
...
% inputs_c = cellfun(@(row) control_law(row(2:4)), tmp, 'UniformOutput', false);
% inputs_c = cell2mat(inputs_c);
% inputs_c = reshape(inputs_c, [], 3);
% 
% inputs_n = cellfun(@(row) random_process(row(1)), tmp, 'UniformOutput', false);
% inputs_n = cell2mat(inputs_n);
% inputs_n = reshape(inputs_n, [], 3);
...
% hold on
% grid on
% plot(x, inputs_c(:, i), 'b', 'linewidth', 1.5)
% plot(x, inputs_n(:,i), 'r', 'linewidth', 1.5)
% plot(x, inputs_n(:,i) + inputs_c(:, i), 'k', 'linewidth', 1.5)
% ylabel(sprintf('$u_%d$', i), 'interpreter', 'latex', 'fontsize', 20)
% legend({"Zero", "Control law", "Noise process", "Combined"})
% end
% linkaxes(axs, 'x')
...
writerObj = VideoWriter('lorenz_feedback_horizontal');
...
%%
writerObj = VideoWriter('lorenz_feedback');
