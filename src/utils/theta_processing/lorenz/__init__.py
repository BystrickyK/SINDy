...
def create_basis(data: DynaFrame):
    # identity = pd.Series((data['u'].values*0+1).T[0], data['u'].index, name='1')
    x = data.get_state_vars()
    dx = data.get_state_derivative_vars()
    u = data.get_input_vars()
    trig_basis_part = trigonometric_library(u)
    theta_basis = pd.concat([x, dx, u, trig_basis_part], axis=1)
    return theta_basis
