...
model_cache_name = 'bestModelsNoisy'
...
filter = SpectralFilter(sim_data_x_noisy, dt, plot=False)
filter.find_cutoff_frequencies(offset=[-4, -8], std_thresh=100)
...
for i, target in enumerate(targets):
    ...
    for n in range(n_models):
        ...
        random_idx = np.random.choice(sim_data_train.index, len(sim_data)//10)
...
for i, target in enumerate(targets):
    ...
    for col in range(X.shape[1]):
        ...
        axs[col].vlines(mode, 0, hist[0][mode_idx]*0.5, color='tab:red', linewidth=2)
        modes.append(mode)
    # axs[3].legend(latexify(term_strs), fontsize=14, loc='upper left',
...
latex_output_file = 'model_latex2.txt'
with open(latex_output_file, 'w') as file:
    file.write(latex_output)

model_name = 'identified_model_noisy_bootstrapped'
os.chdir('models')
...
sim_data_xu = pd.concat([sim_data_val.get_state_vars(), sim_data_val.get_input_vars()], axis=1).reset_index(drop=True)
sim_data_dx = sim_data_val.get_state_derivative_vars()
