...
sim_data = pd.concat([sim_data, sim_data[::-1]]).reset_index(drop=True)  # Append a mirror image of the signal
# sim_data.loc[:, 'phi1'] /= (2*np.pi)
dt = sim_data.iloc[1, 0] - sim_data.iloc[0, 0]  # Get the sampling period from the first two samples
