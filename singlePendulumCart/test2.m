...
% Random walk
    ...
    u(u>0.5) = 0.5;
    u(u<(-0.5)) = -0.5;  % ensures that the cart position doesn't go much further than 0.5
    u(end*5/7:end) = 0; % cart returns to 0 towards the end and stops
    u = filter(filt_b, 0.8*filt_a, u);  % smoothens the trajectory (necessary due to the cutoffs
...
odefun = @(t, X)singlePendCart(t, X, u_f, params);
