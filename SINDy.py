...
from utils.signal_processing import StateSignal, ForcingSignal, SignalProcessor
from utils.identification import PI_Identificator
...
# Simulate the system
sys.propagate(1)  # Simulation without forcing, 1 second
# Control law
u0 = lambda t, x: 0.1*(10-x[0]) + rand_walk1(t)
u1 = lambda t, x: 0.1*(10-x[1]) + rand_walk2(t)
u2 = lambda t, x: 0.1*(10-x[2]) + rand_walk3(t)
u_fun = (u0, u1, u2)
sys.propagate_forced(15, u_fun)  # Simulation with forcing, 15 seconds
...
# Create a ProcessedSignal instance - calculate derivatives, add and filter out noise etc.
sig = ProcessedSignal(
...
# Concatenate the columns of state measurements and inputs
xu = pd.concat([x, u], axis=1)
# Create all polynomials of up to 3rd order (all combinations of x,u)
theta = poly_library(xu, poly_orders=(1, 2, 3, 4))

# Truncate the signal by removing elements from both ends of the signal
...
# Define the control law used in validation simulation
u0 = lambda t, x: rand_walk1(t)*0.1
u1 = lambda t, x: rand_walk2(t)*0.1
u2 = lambda t, x: rand_walk3(t)*0.1
u_fun = (u0, u1, u2)

# Train a number of models, each model is trained using a different set of hyperparameters
...
#%%
# Calculate the RSS, AIC/BIC of each identified model
