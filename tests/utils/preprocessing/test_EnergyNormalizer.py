# %%
from src.utils.preprocessing.EnergyNormalizer import EnergyNormalizer
import numpy as np
import pandas as pd
import pytest
...
class TestEnergyNormalizer(object):
    ...
    def test_calculate_energies_equality_correct(self, resource):
        real = resource.calculate_energies_()
        expected = [5, 55]
        # check equality
        np.testing.assert_equal(real, expected)

    def test_normalize_energy_is_one(self, resource):
        resource.normalize()
        real = resource.calculate_energies()
        expected = 1
        np.testing.assert_equal(real, expected)
