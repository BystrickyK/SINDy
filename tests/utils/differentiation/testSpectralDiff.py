...
for step in steps:
    ...
    dx_from_clean_spectral = compute_spectral_derivative(x, step*dt, mirroring=True)
    dx_from_clean_findiff = compute_finite_diff(x, step*dt)
    if step == 1:
        compare_signals3(np.array(dx.iloc[:,0]).reshape(-1, 1),
                         dx_from_clean_spectral[:,0].reshape(-1, 1),
                         dx_from_clean_findiff[:,0].reshape(-1, 1),
                         ['Real', 'Spectral', 'Finite Difference'],
                         # ylabels=['$\dot{x}_1 \; [m\; s^{-1}]$', '$\dot{x}_2 \; [rad\; s^{-1}]$'],
                         ylabels=['$\dot{x}_1 \; [m\; s^{-1}]$'],
                         title_str='Spectral derivative from clean data')
        plt.xlim([77000//step, 80000//step])
...
#                  title_str='Spectral derivative from filtered data')
    ...
    err_spectral = dx - dx_from_clean_spectral
    err_findiff = dx - dx_from_clean_findiff
